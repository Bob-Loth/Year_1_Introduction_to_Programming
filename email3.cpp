...
int main()
{
  ...
  if (!fileValid(iFileName)) iFileName = "fileContainingEmails.txt";
  
  processLines(iFileName); // opens file, checks, outputs lines, and closes file 

  dFileName = "copyPasteMyEmails.txt";
  
  cout << "Enter output filename [default: " << dFileName << "][press ENTER for default]: ";
  getline(cin,oFileName);
  if (oFileName.length() == 0) oFileName = "copyPasteMyEmails.txt";
  else if (!fileValid(oFileName)) oFileName = iFileName;
