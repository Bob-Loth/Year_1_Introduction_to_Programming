...
{
  //Data
  string objective = "Produce a list of valid, nonduplicate email addresses from a given file, and display them according to the specifications.";
  ...
  oFileName = getFileName("output", oFileName);          
...
void isDuplicate (deque<string>& nonDupe, string aEmail)
{
  ...
  else
  {
    ...
    if (!duplicateFound) nonDupe.push_back(aEmail);
  }//else second+ time
}
...
void outputList(deque<string> nonDupe, string iFileName, string oFileName)
{
  ...
  if (nonDupe.size()==0) cout << endl << "There were no valid email addresses in "<<iFileName<<".  " << "Please check that the file name is correct, ends in .txt, and it contains email addresses.";
  else
  {
    cout << endl << endl << "You can now copy and paste the contents of "<< oFileName << " into any fields that require multiple email addresses. \n";
