...
int main()
{
  ...
  nonDupe.push_front("List of Emails: ");
  ...
  // opens file, checks for valid, nonduplicate emails, saves emails to collection, and closes file 
  processLines(nonDupe, iFileName); 

  //output non-duplicate emails from collection
  cout << nonDupe[0] << " ";
  
  for (i=1; i < nonDupe.size()-1; i++)
  {
    cout << nonDupe[i] << "; " <<endl;
  }
...
void isDuplicate (deque<string>& nonDupe, string aEmail)
{
  ...
  for (i=0; i < nonDupe.size()-1; i++)
  {  
    if (changeCase(aEmail) == changeCase(nonDupe[i])) 
    {
