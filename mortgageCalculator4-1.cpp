...
struct MortgageInfo // contains data items to calculate mortgage payment
  {
    ...
    double monthlyPayment; // the monthly mortgage payment to pay off the loan
...
int main()
{
  ...
  bool clsprg = true; //closes program if false 5 times in the password loop
  int password; // student id number correct value is 1684749
  ...
  for (i=0; i<=5; i++)
  {
     ...
     if (password != 1684749) clsprg = false;
  }
